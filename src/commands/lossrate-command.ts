...
class LossRateCommand extends Command {
  constructor() {
    super("lossrate", {
      cooldown: 1000,
      ratelimit: 1
  ...
  pattern: RegExp = /\[\[(lossrate|lr|losses|loss)\/(all|warrior|support|specialist|assassin)\]\]/i;
  ...
  topWins(message: Message): any {
    ...
    let response = "Top 10 Loss Rates in the last 7 days (with >10% pick+ban rate):";
  ...
  topWinsByRole(message: Message, role: string): any {
    ...
    let response = `Top 10 ${_.startCase(role)} Loss Rates in the last 7 days:`;
    response += renderWinRateBarChart(top10);
...
module.exports = LossRateCommand;